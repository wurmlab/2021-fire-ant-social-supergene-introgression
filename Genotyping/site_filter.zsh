...
## filter 1 VCF
## $HOME/scripts/site_filter.zsh $OUTPUTFOLDER/vcf-genotyping/raw/{}.vcf.gz $HICOVBED $SSRBED $OUTPUTFOLDER/vcf-genotyping/filtered MINALTN $REF.capitalized.fa
...
zcat $VCFfile \
  | vcfintersect -v -l -b $FILTER1 \
  | vcfintersect -v -l -b $FILTER2 \
  | vcffilter -f 'QUAL > 30' \
  | bcftools view -e "NUMALT=1 & ((INFO/SRF)<=($MINALTN/2) | (INFO/SRR)<=($MINALTN/2))" - \
  | vcfallelicprimitives --keep-info --keep-geno -t DECOMPOSED \
  | vcffixup - | vcfstreamsort | vt normalize -n -r $REF -q - \
  | vcfuniqalleles | vcfnoindels | vcfnumalt - \
