...
paths <- c(
  "tmp/2020-07-16-filter_stats/subsampled_pre_filter_gt",
  "tmp/2020-07-16-filter_stats/subsampled_het_filter_gt",
  "tmp/2020-07-16-filter_stats/subsampled_post_filter_gt"
)

df_names <- c(
  "Before filtering",
  "After turning heterzygous to missing",
  "After removing sites with >25% missing genotypes"
)

samples <- read.table("tmp/2020-07-16-filter_stats/samples")
samples <- as.character(samples$V1)

gt <- lapply(1:3, function(i) read_gt(path = paths[i], samples = samples, df_name = df_names[i]))

gt <- do.call(rbind, gt)

# Get all GT possibilities
gt_table <- gather(gt, sample, gt, -df_name)
...
pdf("tmp/2020-07-16-filter_stats/het_per_site.pdf", height = 8, width = 12)
ggplot(het_per_site) +
...
pdf("tmp/2020-07-16-filter_stats/het_per_sample.pdf", width = 20, height=11)
...
pdf("tmp/2020-07-16-filter_stats/missing_per_site.pdf", height = 8, width = 12)
...
pdf("tmp/2020-07-16-filter_stats/missing_per_sample.pdf", width = 20, height=11)
...
gather(gt, sample, gt, -df_name) %>%
  mutate(df_name = factor(df_name, levels  = df_names)) %>%
  mutate(sample = as.factor(sample, levels = het_ordered_samples)) -> long_gt
...
write.csv(gt_summary,
          file      = "tmp/2020-07-16-filter_stats/filter_subset_summary_per_sample.csv",
          row.names = FALSE)
